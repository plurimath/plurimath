    ...
    module Function
      class Unitsml < Core
        ...
        def to_mathml_without_math_tag(intent, options:)
          ...
          mathml["unitsml"] = true
          options[:unitsml_xml] ? wrapped_unitsml_xml(mathml, @unitsml) : mathml
        end
        ...
        def formula(options)
          @unitsml.to_plurimath(options)
        end

        def wrapped_unitsml_xml(mathml, unitsml)
          xml = Plurimath.xml_engine.load("<mrow>#{unitsml.to_xml}</mrow>")
