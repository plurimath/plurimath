...
Common values are 2 (binary), 8 (octal), 10 (decimal, default) and 16 (hexadecimal).
Currently, only bases `2`, `8`, `10`, and `16` are supported.
Passing any other value for `base` raises `Plurimath::Math::InvalidFormatterBaseError`.
+
.Rendering numbers in different bases
[example]
====
"10"    with `base: 2`  => "0b1010"
"9"     with `base: 8`  => "0o11"
"255"   with `base: 16` => "0xff"
"10.75" with `base: 2`  => "0b10,10.1001011" (both integer and fractional parts converted, with default grouping)
====
...
When set to an empty string or `nil`, no prefix is rendered.
+
...
If present, a postfix is appended to the converted integer instead of using any base prefix.
...
When `true` and `base: 16`, hexadecimal *digits* (`a`-`f`) in the converted output
are rendered using uppercase letters. Prefixes and postfixes are not modified.
...
+
NOTE: The fractional-part conversion operates on the fractional *digit sequence* (e.g. `"10.75"` uses `"75"` as the fractional digits),
converting that sequence to the requested base. This matches the current implementation and test expectations.
...
[example]
====
Controls the number of fractional digits in the converted base (pads when needed, but does not truncate if the converted value is longer):
+
"10.75" with `base: 2, precision: 4` => "0b10,10.1001011" (converted fractional part is longer than precision)
"0.5"   with `base: 16, precision: 6` => "0x0.500000" (padded to 6 fractional digits in base 16)
====
...
+
"10.75" with `base: 2, fraction_group_digits: 4, fraction_group: " ", group_digits: 10` => "0b1010.1001 011"
"0.1"   with `base: 16, fraction_group_digits: 2, fraction_group: "'", precision: 8, group_digits: 10` => "0x0.10'00'00'00"
====
...
"10.75"       with `base: 2, digit_count: 5, group_digits: 10` => "0b1010.1000"
====
