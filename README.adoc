...
`base`:: (`Numeric` value)
Sets the numeric base (radix) used to render the *integer* part of the number.
Common values are 2 (binary), 8 (octal), 10 (decimal, default) and 16 (hexadecimal).
+
.Rendering integers in different bases
[example]
====
...
When `true` and `base: 16`, the entire formatted integer (including any prefix or postfix)
is rendered using uppercase letters. It has no effect for other bases.
+
.Uppercase hexadecimal output
[example]
====
"48879" with `base: 16, hex_capital: true` => "0XBEEF"
====

NOTE: When `base` is not 10, only the integer part of the value is converted;
the fractional part is ignored for base conversion and no decimal separator is rendered.
