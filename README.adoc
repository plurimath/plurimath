...
* Separator options (`decimal`, `group`, `fraction_group`, `base_prefix`,
  ...
  For `padding`, `nil` falls back to the default `0` pad character instead.
  Boolean values raise `Plurimath::ConfigurationError`.
* Enumerated options (`notation`, `number_sign`, `exponent_sign`,
  `hex_capital`) accept their documented values as `Symbol` or `String`;
  unrecognized `String`/`Symbol` values fall back to the default behavior.
  Boolean values raise `Plurimath::ConfigurationError`, except for
  `hex_capital`, where `true`/`false` are documented values.
* Invalid number input raises `Plurimath::Formatter::InvalidNumber`.
