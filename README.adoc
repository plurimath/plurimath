...
rendering fails, a `Plurimath::RenderingError` is raised; its message points you
to `lasem-doctor`. A malformed formula surfaces as `Plurimath::Math::ParseError`
from the MathML step, not as `RenderingError`.
