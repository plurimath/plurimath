...
module Plurimath
  class Latex
    module TransformRules
      def self.included(transform_class)
        transform_class.extend(ClassMethods)
        rule_constants = -> { transform_class.constants_class }
        rule_constant = ->(name) { rule_constants.call.const_get(name) }

        transform_class.class_eval do
          ...
          rule(environment: simple(:environment),
            ...
            open_paren = rule_constant.call(:MATRICES)[environment.to_sym]
            Utility.get_table_class(environment).new(
              ...
              Utility.symbols_class(
                rule_constant.call(:MATRICES_PARENTHESIS)[open_paren&.to_sym]&.to_s, lang: :latex, table: true
              ),
              {},
            )
          end
          ...
          rule(environment: simple(:env),
               expression: simple(:expr)) do
            open_paren = rule_constant.call(:MATRICES)[env.to_sym]
