  ...
  class Latex
    module ParsingRules
      def self.included(parser_class)
        parser_class.extend(ClassMethods)
        rule_constants = -> { parser_class.constants_class }
        rule_constant = ->(name) { rule_constants.call.const_get(name) }

        parser_class.class_eval do
          def rule_cache
            self.class.rule_cache
          end
          ...
          rule(:numeric_values) do
            arr_to_expression(rule_constant.call(:NUMERIC_VALUES), :numeric_values)
          end

          rule(:underover_classes) do
            arr_to_expression(rule_constant.call(:UNDEROVER_CLASSES), :binary)
          end

          rule(:math_operators_classes) do
            arr_to_expression(rule_constant.call(:MATH_OPERATORS), :unary_functions)
          end

          rule(:lparen) do
            arr_to_expression(rule_constants.call.parenthesis.keys, :lparen)
          end

          rule(:rparen) do
            arr_to_expression(rule_constants.call.parenthesis.values.flatten, :rparen)
          end

          rule(:left_parens) do
            arr_to_expression(rule_constant.call(:LEFT_RIGHT_PARENTHESIS).keys, :left_paren)
          end

          rule(:right_parens) do
            arr_to_expression(rule_constant.call(:LEFT_RIGHT_PARENTHESIS).keys, :right_paren)
          end

          rule(:environment) do
            arr_to_expression(rule_constant.call(:MATRICES).keys, :environment)
          end
          ...
          rule(:symbol_text_or_integer) do
            str('"').as(:symbol) |
              ...
              (any.present? >> rparen.absent? >> symbol_class_commands) |
          ...
          def arr_to_expression(array, name)
            ...
            rule_cache[:new_hash][name] ||= array.reduce do |expression, expr_string|
      ...
      module ClassMethods
        attr_reader :constants_class

        def use_constants(constants_class)
