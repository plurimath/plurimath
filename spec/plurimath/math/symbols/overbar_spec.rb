  ...
  describe "All language conversion specs" do
    ...
    context "Matches all conversion for the Symbol Plurimath::Math::Symbols::Overbar" do
      ...
      it "matches LaTeX string" do
        expect(klass.to_latex).to eq("\\overbar")
      end

      it "matches UnicodeMath string" do
        expect(klass.to_unicodemath).to eq("̅")
      end

      it "matches OMML string" do
        expect(klass.to_omml_without_math_tag(true)).to eq("&#x305;")
      end

      it "matches MathML string" do
        string = dump_ox_nodes(klass.to_mathml_without_math_tag(false)).strip
        expect(string).to eq("<mi>&#x305;</mi>")
      end

      it "matches HTML string" do
        expect(klass.to_html).to eq("&#x305;")
      end
    end
  end
end
