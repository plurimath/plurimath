...
[example]
====
"32" with `padding_digits: 6, padding: " "` => "    32"
====

`padding_digits`:: (`Numeric` value)
Minimum number of integer digits to render. Missing integer digits are inserted
before grouping using the configured `padding` character. Mutually exclusive
with `padding_group_digits`.
+
.Padding to a fixed integer width
[example]
====
"32"    with `padding_digits: 6` => "000032"
"32123" with `padding_digits: 6` => "032123"
...
`padding` character. Mutually exclusive with `padding_digits`.
+
.Padding to an integer width multiple
[example]
====
"32"    with `padding_group_digits: 4` => "0032"
"32123" with `padding_group_digits: 4` => "00032123"
====
