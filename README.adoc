...
`localized_number` takes precedence, then the formatter instance's `precision`
value, then any `precision` provided through the formatter symbols or `format`
hash.
...
`:plus` to add a leading `+` to positive values. Negative values keep their
normal `-` sign. The sign is applied before base prefixes and before notation
rendering.
+
...
----
{
  ...
  base_prefix: "0x",        # prefixes non-decimal base output
  base_postfix: nil,        # appends a base postfix instead of a prefix when set
...
setting controls how Plurimath emits deprecation notices when a deprecated
feature is used.

[source,ruby]
----
Plurimath.configure do |config|
  config.deprecation.behavior = :warn
end
----

Supported behaviors are:

`warn`:: Default behavior. Emits one warning per deprecated feature.

`raise`:: Raises `Plurimath::DeprecationError` instead of warning.

`silence`:: Suppresses deprecation warnings.
