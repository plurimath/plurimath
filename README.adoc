...
the iteration index shadows any outer binding of the same name, and ranges
are limited to 1,000,000 steps.

The constant is each syntax's recognized pi symbol: textual `pi` in
AsciiMath, LaTeX, and UnicodeMath, and the `π`/`&#x3c0;` forms everywhere,
including HTML. Markups that can write `π` distinctly treat plain-text `pi`
as an ordinary variable, so MathML `<mi>pi</mi>` and OMML text runs resolve
through bindings. In HTML input, letters are individual identifiers, so
plain `pi` means `p*i`.
...
Binding values must be real `Numeric` values. `pi` is a reserved constant in
every input syntax and always resolves to its numeric value; bindings such as
`pi: 3` are ignored rather than overriding it.
