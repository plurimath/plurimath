...
The Plurimath gem provides a Command Line Interface (CLI) tool for converting between different math formats.
...
4. `-t, --output-format <FORMAT>` => Specifies the output format type. Defaults to "mathml".
5. `-m, --math-rendering` => Renders the converted equation as a math zone display tree. Boolean option (true/false).
6. `-d, --display-style` => Specifies the DisplayStyle for OMML and MathML outputs only. Boolean option (true/false).
7. `-s, --split-on-linebreak` => Splits MathML and OMML output into multiple equations. Boolean option (true/false).
...
5. -d, --display-style: Optional. Enables display style MathML conversion (boolean).
